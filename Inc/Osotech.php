        ...
        $this->response = self::alert_msg("Authentication Failed, Please Check your Connection and Try again!","danger").self::loadOsotechCaptcha();
        ...
        $this->response = self::alert_msg("Please fill in all the required fields and Try again!","danger").self::loadOsotechCaptcha();
        }elseif (!self::is_Valid_Email($stu_email)) {
        $this->response = self::alert_msg("$stu_email is not a valid e-mail address, Please check and Try again!","danger").self::loadOsotechCaptcha();
        }elseif (self::check_single_data("visap_student_tbl","stdEmail",$stu_email)) {
        $this->response = self::alert_msg("$stu_email is already taken on this portal!","danger").self::loadOsotechCaptcha();
        }elseif (self::check_single_data("visap_staff_tbl","staffEmail",$stu_email)) {
        $this->response = self::alert_msg("You cannot register with this $stu_email on this portal, its already taken!","danger").self::loadOsotechCaptcha();
        }
        elseif (!self::validate_Mobile_Number($stu_phone)) {
        $this->response = self::alert_msg("This Phone Number $stu_phone format is not allowed, Use this format: +234**********!","danger").self::loadOsotechCaptcha();
        ...
        // code...
        $this->response = self::alert_msg("You have entered an incorrect Card Pin! Re-check Your card Pin & try again...","danger").self::loadOsotechCaptcha();
        }else{
        ...
        if ($pinStatus =='1') {
        // code...
        $this->response = self::alert_msg("DO NOT PLAY SMART! This Card has been Used!","danger").self::loadOsotechCaptcha();
        }else{
        ...
        $this->dbh->rollback();
        $this->response  = self::alert_msg("Error Occurred!: Error Info: ".$e->getMessage(),"danger").self::loadOsotechCaptcha();
        }
        }
        }else{
        //show the user that The PIN he/she enters is not found
        $this->response = self::alert_msg("The Card PIN you entered does not exists! check the Pin and try again...","danger").self::loadOsotechCaptcha();
        }
        }
        return $this->response;
        unset($this->dbh);
